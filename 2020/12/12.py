...
class Ship:
    ...
    def step(self):
        ...
        if inst in 'NSEW':
            ...
            self.turn(val * (1 if inst == 'R' else -1))
        elif inst == 'F':
            self.move(val)
        self.curr += 1
        return self
    ...
    def move(self, val, bearing = None):
        if bearing is None:
    ...
    def turn(self, angle):
        ...
        dir = angle // 90
        directions = 'ESWN'
        curr_dir = directions.find(self.bearing)
        curr_dir = (curr_dir + dir) % 4
        self.bearing = directions[curr_dir]

    def get_manhattan_distance(self):
        return abs(self.x) + abs(self.y)
    
    def __str__(self):
        return f'x: {self.x} y: {self.y} bearing: {self.bearing} manhattan distance: {self.get_manhattan_distance()}'
